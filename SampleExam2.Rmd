...
There are three airports in the city of New York. These are coded as the `origin` variable. 
  ...
  In R, calculate the number of flights from each of these airports by **each** of these `carrier`. (i.e. construct a 2-by-2 `table` or a summary `data.frame`). Which one of the three airports does Virgin America Airline NOT operating from?
...
```
 ...
 + The Normal curve appears to fit poorly based on the qq-plot.
 + There are more values of 
...
table(small_flights$carrier, small_flights$origin) %>% chisq.test()
```


## Question 11: Chi-square test 2

It is postulated that AA spreads its flights over three airports in ratios of 10:40:45. 
